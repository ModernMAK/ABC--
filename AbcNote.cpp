...
#include "AbcNote.h"
void AbcNote::init(std::string abc) {
	//Serial.print("ABC : ");
	//Serial.print(abc);
	//Serial.print(" -> ");
...
AbcNote::AbcNote(std::string abc) {
	init(abc);
}
...
std::string AbcNote::toAbcString()
{
...
std::string AbcNote::getAbcStringFromPitch(UniquePitch uP, bool isMiddlePitch)
{
		std::string temp = "";
...
UniquePitch AbcNote::getUniquePitchFromAbc(std::string abc) {
	char cPitch = 'C';
...
int AbcNote::getOctaveFromAbc(std::string abc) {
	int o = 4;
	byte state = 0;
	for (size_t i = 0; i < abc.length(); i++)
	{
...
std::string AbcNote::getAbcFromOctave(int i)
{
	i -= 4;//Shift to C
	std::string str = "";
	while (i < 0) {
...
Fraction* AbcNote::getTimingFromAbc(std::string abc) {
	std::string
		myNum = "1",
		myDenom = "1";
	byte state = 0; //0 = Looking, 1 = Nat, 2 = Sharp, 3 = Flat
	...
	for (size_t i = 0; i < abc.length(); i++)
	{
		...
		else 
		{
			...
			{
				myNum = abc.substr(lI, i - lI);
				lI = i;
				state = 2;
			}
			else if (state == 4) 
			{
				myDenom = abc.substr(lI, i - lI);
				state = 5;
			}
		}
	}
	if (state == 4)
	{
		myDenom = abc.substr(lI, abc.length() - lI);
	}
	
	return new Fraction(stoi(myNum), stoi(myDenom));
}

std::string AbcNote::getAbcFromTiming(Fraction * abc)
{
	std::string str = "";
